//Eshaani Arvind
//U88155472
//Program to modify project 8 add job function to sort using ordered lists according to priority
...
struct job * add_job (struct job *scheduler, char *job_name, char *user_name, int num_cpus, int num_gpus, int memory, double time, int priority) 
{
    ...
    struct job *cur, *prev;                  //2 pointers to traverse through linked list
    ...
    new_node->time = time;
    //traversing though linked list to compare if nodes are lesser than or equal to new node and find where to place new node
    for (cur = scheduler , prev = NULL; cur != NULL && new_node->priority <= cur->priority; prev = cur, cur = cur->next)
    ;
    if (cur != NULL && new_node->priority == cur->priority)
    {
        new_node->next = cur->next;     //new node is placed in between last duplicate priority and its next node
        cur->next = new_node;
        return scheduler;
    }
    new_node->next = cur;           //else placed before current node
    if (prev == NULL)               //if scheduler is empty, new node is first in list
        return new_node;
    else
    {
        prev->next = new_node;      //new node is placed after previous node
        return scheduler;
    }
}
